...
public class FilesystemContentStore implements ContentStore {
    ...
    public ContentReader getReader(
        ...
        try {
            if (contentRange != null && Files.size(path) != contentRange.getContentSize()) {
        ...
        ContentReader reader = new FileContentReader(path, contentReference);

        if (contentRange != null) {
            reader = new EmulatedRangedContentReader(reader, contentRange);
        }
    ...
    public ContentAccessor writeContent(@NonNull InputStream inputStream) throws UnwritableContentException {
        ...
        return new FileContentAccessor(contentReference);
    }
