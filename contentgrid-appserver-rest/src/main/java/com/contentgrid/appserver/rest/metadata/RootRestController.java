...
public class RootRestController {
    ...
    public ResponseEntity<String> openApiSpecJson(Application application) {
        return ResponseEntity.ok(OpenApiSpecConverter.Writer.toJson(OpenApiSpecConverter.convert(application)));
    }

    @GetMapping(value = "/openapi.yml", produces = MediaType.APPLICATION_YAML_VALUE)
    public ResponseEntity<String> openApiSpecYml(Application application) {
        return ResponseEntity.ok(OpenApiSpecConverter.Writer.toYaml(OpenApiSpecConverter.convert(application)));
    }
