...
class AutomationModelPermissionEvaluator {
    ...
    public boolean isAllowed(ThunkExpression<Boolean> permissionPredicate, AutomationModel model) {
        var result = permissionPredicate.accept(VISITOR, model)
                .assertResultType(Boolean.class);
        return ThunkExpression.maybeValue(result).orElseThrow();
    }

    private static class AutomationModelPermissionVisitor implements ThunkExpressionVisitor<ThunkExpression<?>, AutomationModel> {
        ...
        public Scalar<?> visit(FunctionExpression<?> functionExpression, AutomationModel context) {
            ...
            return switch (functionExpression.getOperator()) {
                case EQUALS -> {
                    ...
                    yield Scalar.of(terms.getFirst().equals(terms.getLast()));
                }
                case NOT_EQUAL_TO -> {
                    assertTwoTerms(terms);
                    yield Scalar.of(!terms.getFirst().equals(terms.getLast()));
                ...
                case IN -> {
                    assertTwoTerms(terms);
                    if (terms.getLast() instanceof CollectionValue<?> collectionValue) {
                        yield Scalar.of(collectionValue.getValue().stream()
                                .map(ThunkExpression.class::cast)
                                .anyMatch(value -> terms.getFirst().equals(value)));
                    }
                    throw new IllegalArgumentException("Operation 'in' requires a collection as right-hand side expression");
                }
                case AND -> {
                ...
                case NOT -> {
                    assertOneTerm(terms);
                    var term = (Scalar<Boolean>) terms.getFirst().assertResultType(Boolean.class);
                    yield Scalar.of(!term.getValue());
                }
                default -> throw new UnsupportedOperationException("Operation %s not implemented"
        ...
        public Scalar<?> visit(SymbolicReference symbolicReference, AutomationModel context) {
            ...
            try {
                ...
                return switch (result) {
                    case null -> Scalar.nullValue();
                    case Long number -> Scalar.of(number);
                    case Double number -> Scalar.of(number);
                    case BigDecimal number -> Scalar.of(number);
                    case Boolean bool -> Scalar.of(bool);
                    case String string -> Scalar.of(string);
                    default -> throw new IllegalArgumentException("Field '%s' has unknown type '%s'"
        ...
        @Override
        public SetValue visit(SetValue setValue, AutomationModel automationModel) {
            var result = setValue.getValue().stream()
                    .map(value -> value.accept(this, automationModel))
                    .collect(Collectors.toSet());
            return new SetValue(result);
        }

        @Override
        public ListValue visit(ListValue listValue, AutomationModel automationModel) {
            var result = listValue.getValue().stream()
                    .map(value -> value.accept(this, automationModel))
                    .toList();
