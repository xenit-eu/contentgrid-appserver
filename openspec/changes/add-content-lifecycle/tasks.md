...
- [ ] 2.1 Add `createContentReferencesTable(DSLContext)` and `dropContentReferencesTable(DSLContext)` methods to
...
- [ ] 2.2 Define `_content_references` table and field references as `DSL.table()` / `DSL.field()` constants in the
...
- [ ] 3.1 Define the `ContentReferenceTracker` interface in the new module with `incrementReference(ContentReference)`
  and `decrementReference(ContentReference)` methods
- [ ] 3.2 Implement `JooqContentReferenceTracker`: `incrementReference` upserts a row (insert or increment count, clear
...
- [ ] 3.3 Implement `DeferredContentReferenceTracker` that delegates `incrementReference` directly and registers
  `decrementReference` as a `TransactionSynchronizationManager.afterCommit()` callback
- [ ] 3.4 Write unit tests for `JooqContentReferenceTracker` covering: first increment (creates row), second increment (
  increments count, clears deletion marker), decrement to non-zero (only decrements), decrement to zero (sets
  `marked_for_deletion_at`)
- [ ] 3.5 Write unit tests for `DeferredContentReferenceTracker` verifying that decrement is not called when the
...
- [ ] 5.1 Update `ContentAttributeModificationValidator` to collect content IDs that are being set to null; expose
  them (e.g., return them from `validate()` or accumulate in a field)
- [ ] 5.2 In `DatamodelApiImpl.update()` and `updatePartial()`, after the query engine operation, call
  `contentReferenceTracker.decrementReference()` for each collected dereferenced content ID
- [ ] 5.3 Write tests verifying decrement is called when content is cleared and not called when attribute was previously
  empty

## 6. Domain Integration — Entity Delete (Decrement)

- [ ] 6.1 Add a helper in `DatamodelApiImpl` that extracts all non-null `ContentReference` values from an
  `InternalEntityInstance` by scanning the `Application` model for `ContentAttribute` instances
- [ ] 6.2 In `DatamodelApiImpl.deleteEntity()`, after `queryEngine.delete()` returns the deleted entity, call
  `contentReferenceTracker.decrementReference()` for each extracted content reference
- [ ] 6.3 Write tests for the delete path: entity with multiple content attributes (all decremented), entity with no
  content attributes (no decrement), entity with mix of populated and empty content attributes

## 7. ContentDeletionJob

- [ ] 7.1 Implement `ContentReferenceVerificationQuery` that, given a `content_id`, queries all entity tables with
  content attributes and returns true if any reference exists
- [ ] 7.2 Implement `ContentDeletionJob`: query `_content_references` where
...
- [ ] 7.3 Record metrics: `content.deletion.success`, `content.deletion.failure`, `content.deletion.drift`
- [ ] 7.4 Expose the job as a Spring Boot `CommandLineRunner` or `ApplicationRunner` for K8s CronJob invocation
- [ ] 7.5 Write unit tests for `ContentDeletionJob`: candidate past grace period and unreferenced (deleted), candidate
...
- [ ] 9.1 Document the backfill SQL pattern (UNION ALL across all entity content columns) in the module README or a
  runbook
- [ ] 9.2 Consider whether the backfill should be an optional Flyway migration or a manual operation; document the
  chosen approach
